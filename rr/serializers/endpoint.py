...
class EndpointSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Endpoint
        fields = ['id', 'sp', 'type', 'binding', 'location', 'response_location', 'index', 'is_default', 'created_at',
                  'updated_at', 'end_at', 'validated', 'status']
        read_only_fields = ['created_at', 'updated_at', 'end_at', 'validated', 'status']

        validators = [
            UniqueTogetherValidator(
                queryset=Endpoint.objects.filter(end_at=None),
                fields=['sp', 'type', 'binding', 'location']
            )
        ]
    ...
    def create(self, validated_data):
        ...
        logger.info("Endpoint added for {sp} by {user}"
                    .format(sp=endpoint.sp, user=user))
        return endpoint
