...
from rr.forms.testuser import TestUserDataForm, TestUserForm, TestUserUpdateForm
...
def testuser_list(request, pk):
    ...
    **Template:**
    ...
    return render(
        request,
        "rr/testuser.html",
        {"object_list": testusers, "object_list_external": testusers_external, "form": form, "object": sp},
    )
    ...
    if form.is_valid():
        ...
        for number in range(1, number_of_users + 1):
            if number_of_users == 1:
                ...
                password = hashlib.sha256((form.cleaned_data["password"] + str(number)).encode("utf-8")).hexdigest()
            if not TestUser.objects.filter(username=username, end_at=None).exists():
                testuser = TestUser.objects.create(
                    sp=sp, username=username, password=password, firstname=firstname, lastname=lastname, end_at=None
                )
    ...
    for key, value in request.POST.dict().items():
        if value == "on":
            ...
            if testuser.sp == sp:
                ...
                messages.add_message(request, messages.INFO, _("Test user removed: ") + testuser.username)
    ...
    for key, value in request.POST.dict().items():
        if value == "on":
            ...
            if sp in testuser.valid_for.all() and testuser.sp != sp:
                ...
                logger.info(
                    "External test user {username} removed from {sp}".format(username=testuser.username, sp=sp)
                )
                messages.add_message(request, messages.INFO, _("External test user removed: ") + testuser.username)
...
def testuser_attribute_data(request, pk):
    ...
    **Template:**
    ...
    if (
        ...
        and not ServiceProvider.objects.filter(
            (Q(admins=request.user) | Q(admin_groups__in=request.user.groups.all())), pk=testuser.sp.pk, end_at=None
        )
        .distinct()
        .last()
    ):
    ...
    if request.method == "POST":
        ...
        if "reset_userdata" in request.POST:
            ...
            messages.add_message(
                request, messages.INFO, _("Name based data reset for test user: ") + testuser.username
            )
        if "reset_otherdata" in request.POST:
            generate_user_data(testuser, userdata=False, otherdata=True)
            form = TestUserDataForm(user=testuser)
            messages.add_message(request, messages.INFO, _("User data reset for test user: ") + testuser.username)
        if "update_user" in request.POST:
            user_update_form = _update_user(request, testuser)

    return render(
        request,
        "rr/testuser_attribute_data.html",
        {"form": form, "user_update_form": user_update_form, "object": testuser.sp, "testuser": testuser},
    )
        ...
        for field in form:
            ...
            for value in values:
                if value:
                    ...
                    TestUserData.objects.create(user=testuser, attribute=attribute, value=value)
        messages.add_message(request, messages.INFO, _("Test user attributes updated: ") + testuser.username)
    return form
...
def _update_user(request, testuser):
    ...
    if user_update_form.is_valid():
        ...
        user_update_form = TestUserUpdateForm(instance=testuser, sp=testuser.sp, admin=request.user)
        messages.add_message(request, messages.INFO, _("User info updated: ") + testuser.username)
