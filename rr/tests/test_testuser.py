...
class TestUserTestCase(TestCase):
    def setUp(self):
        ...
        self.oidc_sp = ServiceProvider.objects.create(entity_id="test:entity:oidc", service_type="oidc", test=True)
        ...
        SPAttribute.objects.create(
            attribute=self.attr_eppn, sp=self.oidc_sp, reason="User identification", oidc_userinfo=True
        )
        self.testuser = TestUser.objects.create(
            sp=self.saml_sp, username="jack", password="secret", firstname="Jack", lastname="Tester"
        )
    ...
    def test_testuser_view_denies_anonymous(self):
        ...
        self.assertRedirects(
            response, reverse("login") + "?next=" + reverse("testuser-list", kwargs={"pk": self.saml_sp.pk})
        )
    ...
    def test_testuser_view_create(self):
        ...
        response = self.client.post(
            reverse("testuser-list", kwargs={"pk": self.saml_sp.pk}),
            {
                "add_testuser": "ok",
                "username": "jjohn",
                "password": "secretword",
        ...
        self.assertEqual(
            TestUserData.objects.filter(attribute=self.attr_cn, user=test_jjohn, value="John Johnson").count(), 1
        )
        self.assertEqual(
            TestUserData.objects.filter(attribute=self.attr_eppn, user=test_jjohn, value="jjohn@example.org").count(),
            1,
        )
    ...
    def test_testuser_view_create_custom_scope(self):
        ...
        test_jjohn = TestUser.objects.get(username="jjohn")
        self.assertEqual(
            TestUserData.objects.filter(attribute=self.attr_eppn, user=test_jjohn, value="jjohn@test.org").count(), 1
        )
    ...
    def test_testuser_view_change_data(self):
        self.client.force_login(self.user)
        response = self.client.post(
            reverse("testuser-attribute-data", kwargs={"pk": self.testuser.pk}),
            {"update_user": "ok", "password": "", "firstname": "John", "lastname": "Johnson", "valid_for": ["1", "3"]},
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_testuser_view_change_password(self):
        ...
        response = self.client.post(
            reverse("testuser-attribute-data", kwargs={"pk": self.testuser.pk}),
            {"update_user": "ok", "password": "word", "firstname": "John", "lastname": "Johnson", "valid_for": ["1"]},
        )
        self.assertEqual(response.status_code, 200)
        self.assertIn("User info updated: jack", response.content.decode())
        test_jack = TestUser.objects.get(username="jack")
    ...
    def test_testuser_view_userdata_change(self):
        ...
        response = self.client.post(
            reverse("testuser-attribute-data", kwargs={"pk": self.testuser.pk}),
            {"save_data": "ok", "cn": "Team User", "eduPersonPrincipalName": "teamuser@example.net"},
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_testuser_view_invalid_userdata_change(self):
        ...
        response = self.client.post(
            reverse("testuser-attribute-data", kwargs={"pk": self.testuser.pk}),
            {"save_data": "ok", "cn": "Team User", "eduPersonPrincipalName": "teamuser"},
        )
        self.assertEqual(response.status_code, 200)
