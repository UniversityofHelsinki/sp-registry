...
from _socket import gaierror, getaddrinfo
...
class Command(BaseCommand):
    def add_arguments(self, parser):
        parser.add_argument("-s", action="store_true", dest="check_saml", help="Check SAML services")
        parser.add_argument("-l", action="store_true", dest="check_ldap", help="Check LDAP services")
        parser.add_argument("-o", action="store_true", dest="check_oidc", help="Check OIDC services")
        parser.add_argument(
            "-a",
            action="store_true",
            dest="test_all",
    ...
    def handle(self, *args, **options):
        ...
        if test_all:
            services = ServiceProvider.objects.filter(end_at=None, history=None).order_by("service_type", "entity_id")
        else:
            services = ServiceProvider.objects.filter(end_at=None, history=None, production=True).order_by(
    ...
    def _check_saml_service(self, sp):
        ...
        for endpoint in set(endpoints):
            try:
                ...
                self.stdout.write(
                    "Address does not resolve | SAML entity: %s | address: %s" % (sp.entity_id, endpoint)
                )
    ...
    def _check_oidc_service(self, sp):
        ...
        for redirecuri in set(redirecturis):
            try:
                ...
                self.stdout.write(
                    "Address does not resolve | OIDC entity: %s | address: %s" % (sp.entity_id, redirecuri)
                )
    ...
    def _check_ldap_service(self, sp):
        for server_address in sp.server_names.splitlines():
            try:
                ...
                self.stdout.write(
                    "Address does not resolve | LDAP entity: %s | address: %s" % (sp.entity_id, server_address)
                )
