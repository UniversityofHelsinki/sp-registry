...
class TestuserTestCase(APITestCase):
    def setUp(self):
        ...
        self.object = TestUser.objects.create(
            sp=self.saml_sp, username="jack", password="secret", firstname="Jack", lastname="Tester"
        )
        self.admin_object = TestUser.objects.create(
            sp=self.admin_sp, username="adminjack", password="adminsecret", firstname="Jack", lastname="Admin"
        )
    ...
    def test_testuser_create_user_with_attributes(self):
        ...
        data["attributes"] = [
            {"attribute": "eduPersonPrincipalName", "value": "exorg@example.org"},
            {"attribute": "cn", "value": "Ogre"},
        ]
    ...
    def test_testuser_create_user_with_valid_for_without_permission(self):
        data = self.create_data
        data["valid_for"] = ["test:entity:1", "test:entity:2"]
        response = self._test_create(user=self.user, data=data)
    ...
    def test_testuserdata_create_with_user(self):
        self._initialize_test_user_data()
        data = {"user": self.object.pk, "attribute": "cn", "value": "Jack"}
        response = self._test_create(user=self.user, data=data)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(len(self.object.attributes.all()), 2)
