...
class SPAdminFormTestCase(TestCase):
    ...
    def test_spadmin_view_denies_anonymous(self):
        ...
        self.assertRedirects(response,
                             reverse('login') + '?next=' + reverse('admin-list', kwargs={'pk': self.user_sp.pk}))
    ...
    def test_spadmin_view_post_invite(self):
        self.client.force_login(self.user)
        response = self.client.post(reverse('admin-list', kwargs={'pk': self.user_sp.pk}),
                                    {'add_invite': 'ok', 'email': 'test@example.org'})
        self.assertEqual(response.status_code, 200)
    ...
    def test_spadmin_view_use_invite(self):
        ...
        self.client.force_login(self.user)
        response = self.client.get(reverse('invite-activate-key', kwargs={'invite_key': key.activation_key}),
                                   follow=True)
        self.assertEqual(response.status_code, 200)
    ...
    def test_spadmin_view_add_group(self):
        self.client.force_login(self.user)
        response = self.client.post(reverse('admin-list', kwargs={'pk': self.user_sp.pk}),
                                    {'add_admin_group': 'ok', 'group': 'newgroup'})
        self.assertEqual(response.status_code, 200)
