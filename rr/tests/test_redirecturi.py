...
class RedirectUriTestCase(TestCase):
    ...
    def test_redirect_uri_view_denies_anonymous(self):
        ...
        self.assertRedirects(
            response, reverse("login") + "?next=" + reverse("redirecturi-list", kwargs={"pk": self.user_sp.pk})
        )
    ...
    def test_redirect_uri_view_post(self):
        ...
        response = self.client.post(
            reverse("redirecturi-list", kwargs={"pk": self.user_sp.pk}),
            {"add_redirecturi": "ok", "uri": "https://example.org/redirect_uri"},
        )
        self.assertEqual(response.status_code, 200)
