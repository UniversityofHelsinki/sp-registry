...
from rr.views_api.serviceprovider import OidcServiceProviderViewSet
...
class ServiceTestCase(APITestCase):
    def setUp(self):
        ...
        self.object = ServiceProvider.objects.create(entity_id="test.service", service_type="oidc", name_en="My Test")
        self.object.admins.add(self.user)
        self.super_user_object = ServiceProvider.objects.create(entity_id="test.entity.1", service_type="oidc")
        self.data = {"id": self.object.id, "entity_id": self.object.entity_id}
        self.create_data = {"entity_id": "NewService", "name_en": "New OIDC ServiceProvider"}
        self.update_data = {"id": self.object.id, "entity_id": self.object.entity_id, "name_en": "Your test"}
        self.url = "/api/v1/services/oidc/"
        self.viewset = OidcServiceProviderViewSet
    ...
    def test_service_provider_create_with_nested_data(self):
        ...
        data = {
            ...
            "name_en": "Created provider",
            "admins": ["superuser", "tester"],
            "admin_groups": ["TestGroup"],
            "contacts": [
                {"type": "technical", "firstname": "Test", "lastname": "User", "email": "tester@example.org"},
    ...
    def test_service_provider_update_with_nested_attribute_Removal(self):
        self.test_service_provider_create_with_nested_data()
        data = {"entity_id": "New.Service", "attributes": [{"attribute": "cn", "reason": "Frog"}]}
        response = self._test_update(user=self.user, data=data, pk=self.sp_id)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
