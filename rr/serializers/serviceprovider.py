    ...
    if "spattributes" in validated_data:
        ...
        if removed_attributes:
            ...
            for attribute in removed_attributes:
                ...
                attribute.save()
                logger.info(
                    "Attribute {attribute} removed from {service} by {user}".format(
                        attribute=attribute.attribute, service=attribute.sp, user=user
                    )
                )
    return modified
...
def _update_certificates(validated_data, instance, user, modified):
    if "certificates" in validated_data:
        ...
        for certificate in certificates:
            certificate_instance, created = Certificate.objects.get_or_create(sp=instance, end_at=None, **certificate)
            if created:
                modified = True
                logger.info("Certificate added for {service} by {user}".format(service=instance, user=user))
        ...
        if removed_certificates:
            ...
            for certificate in removed_certificates:
                ...
                certificate.save()
                logger.info("Certificate removed from {service} by {user}".format(service=certificate.sp, user=user))
    return modified
...
def _update_contacts(validated_data, instance, user, modified):
    if "contacts" in validated_data:
        ...
        for contact in contacts:
            ...
            if created:
                modified = True
                logger.info("Contact added for {service} by {user}".format(service=instance, user=user))
        ...
        if removed_contacts:
            ...
            for contact in removed_contacts:
                ...
                contact.save()
                logger.info("Contact removed from {service} by {user}".format(service=contact.sp, user=user))
    return modified
...
def _update_endpoints(validated_data, instance, user, modified):
    if "endpoints" in validated_data:
        ...
        for endpoint in endpoints:
            ...
            if created:
                modified = True
                logger.info("Endpoint added for {service} by {user}".format(service=instance, user=user))
        ...
        if removed_endpoints:
            ...
            for endpoint in removed_endpoints:
                ...
                endpoint.save()
                logger.info("Endpoint removed from {service} by {user}".format(service=endpoint.sp, user=user))
    return modified
...
def _update_redirecturis(validated_data, instance, user, modified):
    if "redirecturis" in validated_data:
        ...
        for redirecturi in redirecturis:
            ...
            if created:
                modified = True
                logger.info("RedirectUri added for {service} by {user}".format(service=instance, user=user))
        ...
        if removed_redirecturis:
            ...
            for redirecturi in removed_redirecturis:
                ...
                redirecturi.save()
                logger.info("RedirectUri removed from {service} by {user}".format(service=redirecturi.sp, user=user))
    return modified
...
def _update_usergroups(validated_data, instance, user, modified):
    if "usergroups" in validated_data:
        ...
        for usergroup in usergroups:
            ...
            if created:
                modified = True
                logger.info("UserGroup added for {service} by {user}".format(service=instance, user=user))
        ...
        if removed_usergroups:
            ...
            for usergroup in removed_usergroups:
                ...
                usergroup.save()
                logger.info("UserGroup removed from {service} by {user}".format(service=usergroup.sp, user=user))
    return modified


class SPAttributeSerializer(serializers.ModelSerializer):
    attribute = serializers.SlugRelatedField(slug_field="friendlyname", queryset=Attribute.objects.all())
...
class SamlServiceProviderSerializer(serializers.ModelSerializer):
    ...
    nameidformat = serializers.SlugRelatedField(
        many=True, slug_field="nameidformat", queryset=NameIDFormat.objects.filter(public=True), required=False
    )
    ...
    def validate_endpoints(self, value):
        for instance in value:
            ...
            endpoint_validator(
                self.instance, binding, index, is_default, location, endpoint_type, serializers.ValidationError
            )
        return value
    ...
    def create(self, validated_data):
        ...
        if not user.is_superuser and (not admin_groups or not user.groups.filter(name__in=admin_groups).exists()):
            sp.admins.add(user)
        logger.info("Service {service} created by {user}".format(service=sp, user=user))
        if attributes:
            for attribute in attributes:
                spattribute = SPAttribute.objects.create(sp=sp, **attribute)
                logger.info(
                    "Attribute {attribute} requested for {sp} by {user}".format(
                        attribute=spattribute.attribute, sp=sp, user=user
                    )
                )
        if certificates:
            for certificate in certificates:
    ...
    def update(self, instance, validated_data):
        ...
        if changed_data or modified:
            ...
            logger.info("Service {service} updated by {user}".format(service=instance, user=user))
            if modified:
...
class OidcServiceProviderSerializer(serializers.ModelSerializer):
    ...
    grant_types = serializers.SlugRelatedField(
        many=True, slug_field="name", queryset=GrantType.objects.all(), required=False
    )

    oidc_scopes = serializers.SlugRelatedField(
        many=True, slug_field="name", queryset=OIDCScope.objects.all(), required=False
    )

    response_types = serializers.SlugRelatedField(
        many=True, slug_field="name", queryset=ResponseType.objects.all(), required=False
    )
...
class LdapServiceProviderSerializer(serializers.ModelSerializer):
    ...
    usergroups = serializers.SlugRelatedField(
        many=True, slug_field="name", queryset=UserGroup.objects.all(), required=False
    )
