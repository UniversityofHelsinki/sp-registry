...
class ContactSerializer(serializers.ModelSerializer):
    class Meta:
        model = Contact
        fields = ['id', 'sp', 'type', 'firstname', 'lastname', 'email', 'created_at', 'updated_at', 'end_at',
                  'validated', 'status']
        read_only_fields = ['created_at', 'updated_at', 'end_at', 'validated', 'status']

        validators = [
            UniqueTogetherValidator(
                queryset=Contact.objects.filter(end_at=None),
                fields=['sp', 'type', 'firstname', 'lastname', 'email']
            )
        ]
    ...
    def create(self, validated_data):
        ...
        logger.info("Contact added for {sp} by {user}"
                    .format(sp=contact.sp, user=user))
        return contact
