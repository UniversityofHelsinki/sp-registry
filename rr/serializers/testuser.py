...
class TestUserSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        logger.info("Test user {username} added for {sp} by {user}"
                    .format(username=testuser.username, sp=testuser.sp, user=user))
        if valid_for:
            for sp in valid_for:
    ...
    def update(self, instance, validated_data):
        ...
        instance.save()
        logger.info("Test user {username} modified for {sp} by {user}"
                    .format(username=instance.username, sp=instance.sp, user=user))
        if valid_for:
            ...
            for attribute in attributes:
                if 'value' in attribute and attribute['value']:
                    try:
                        ...
                        TestUserData.objects.get_or_create(user=instance,
                                                           attribute=attribute['attribute'],
                                                           value=attribute['value'])
