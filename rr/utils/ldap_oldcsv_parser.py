...
ad = {
    "peopletimestampreaders": ["createTimestamp", "modifyTimestamp"],
    "unitreaders": [
        "hyAccountingCode",
        "hyDepartmentCode",
        "hyDivisionCode",
...
def try_to_make_sure_attribute_exists(friendlyname):
    ...
    if not attribute:
        ...
        Attribute.objects.create(
            name=name, public_saml=0, attributeid=attributeid, friendlyname=friendlyname, public_ldap=1
        )
    else:
...
def parse_ldap_attributes(sp, d, validate, errors):
    ...
    specialanames = list(
        filter(lambda x: len(x) > 0, map(lambda x: x.strip(), (d["Tarvittavat erikoisattribuutit"]).split(" ")))
    )
    groupedanames = []
...
def add_ldap_spadmin(sp, maileppn):
    (adminemail, eppn) = maileppn.split("=")

    admin = User.objects.filter(username=eppn).first()

    if not admin:
        admin = User(username=eppn, password=User.objects.make_random_password(), email=adminemail, is_active=True)
...
def make_usergroup(sp, name, validate):
    ...
    if not UserGroup.objects.filter(sp=sp, name=name).exists():
        if validate:
            UserGroup.objects.create(sp=sp, name=name, validated=timezone.now())
        else:
            UserGroup.objects.create(sp=sp, name=name, validated=None)


def parse_ldap_groups(sp, d, validate):
    """
    ...
    if ug_s == "Kaikki":
        sp.can_access_all_ldap_groups = True
        sp.save()
        return
...
def make_contact(sp, contacttype, namel, email, validate):
    ...
    if (
        not Contact.objects.filter(
            sp=sp, type=contacttype, firstname=firstname, lastname=lastname, email=email
        ).exists()
        and email
    ):
        if validate:
            Contact.objects.create(
                sp=sp, type=contacttype, firstname=firstname, lastname=lastname, email=email, validated=timezone.now()
            )
        else:
            Contact.objects.create(
                sp=sp, type=contacttype, firstname=firstname, lastname=lastname, email=email, validated=None
            )
...
def parse_ldap_contacts(sp, d, validate, errors):
    ...
    if len(adminnames) == len(adminemails):  # normal case
        ...
        errors.append(
            "LDAP contact parser: %s: couldn't make admin email and name lists the same "
            "length, didn't add tech contacts" % sp.name_fi
        )
...
def parse_ldapdict(sp, d, validate, errors):
    ...
    if (
        d["Tallentaako järjestelmä käyttäjien antamat salasanat: Tallennetaanko käyttäjän salasana"]
        == "salasanatallennetaan"
    ):
    ...
    sp.server_names = "\n".join(
        filter(
            lambda x: x != "",
            d["Palvelimien täydelliset nimet välilyönnillä erotettuina (ei ip-osoitetta)"].split(" "),
        )
    )
    ...
    targetgroupmap = {"restricted": "restricted", "hy": "university", "public": "internet"}
        ...
        try:
            ...
            if validate:
                sp = ServiceProvider.objects.create(
                    entity_id=entity_id, name_fi=name, service_type="ldap", validated=timezone.now(), modified=False
                )
            else:
                sp = ServiceProvider.objects.create(
                    entity_id=entity_id, name_fi=name, service_type="ldap", validated=None, modified=True
                )
            if verbosity > 3:
