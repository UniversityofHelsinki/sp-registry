...
def metadata_redirect_uris(sp, validation_date):
    ...
    if validation_date:
        redirect_uris = (
            RedirectUri.objects.filter(sp=sp)
            .filter(Q(end_at=None) | Q(end_at__gt=validation_date))
            .exclude(validated=None)
        )
    else:
...
def oidc_metadata_generator_list(
    validated=True, privacypolicy=False, production=False, test=False, include=None, client_secret_encryption=None
):
    """
