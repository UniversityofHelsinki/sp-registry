        ...
        if user:
            if user.is_active:
                ...
                if redirect_to == request.path:
                    error_message = _("Redirection loop for authenticated user detected. "
                ...
                logger.warning("User {user} with deactivated account tried to log in"
                               .format(user=user))
...
def user_logged_in_callback(sender, request, user, **kwargs):
    ...
    logger.info('login user: {user} via ip: {ip}'
                .format(user=user, ip=ip))
...
def user_logged_out_callback(sender, request, user, **kwargs):
    ...
    logger.info('logout user: {user} via ip: {ip}'
                .format(user=user, ip=ip))
