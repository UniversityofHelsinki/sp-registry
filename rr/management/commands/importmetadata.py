...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('-o', action='store_true', dest='overwrite', help='Overwrite/add to existing SPs with same entity_id')
        ...
        if files:
            for filename in files:
                ...
                if not tree:
                    ...
                    root = tree.getroot()
                    for entity in root:
                        sp, errors = metadata_parser(entity, overwrite, verbosity, validate, disable)
                        for error in errors:
                            print(error)
