...
class ServiceProviderOIDCTechnicalTestCase(TestCase):
    ...
    def test_sp_oidc_view_denies_anonymous(self):
        ...
        self.assertRedirects(
            response, reverse("login") + "?next=" + reverse("oidc-technical-update", kwargs={"pk": self.user_sp.pk})
        )
    ...
    def test_sp_oidc_entity_id_change(self):
        ...
        response = self.client.post(
            reverse("oidc-technical-update", kwargs={"pk": self.user_sp.pk}), {"entity_id": "test:entity:1"}
        )
        self.assertEqual(response.status_code, 200)
        self.assertIn("Client ID already exists", response.content.decode())
        response = self.client.post(
            reverse("oidc-technical-update", kwargs={"pk": self.user_sp.pk}),
            {"entity_id": "https://sp2.example.org/sp/valid"},
            follow=True,
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_sp_oidc_grant_and_response_type(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertNotIn(
            "authorization_code Grant type must be set for code response type.", response.content.decode()
        )
    ...
    def test_sp_jwks(self):
        ...
        self.client.force_login(self.user)
        response = self.client.post(
            reverse("oidc-technical-update", kwargs={"pk": self.user_sp.pk}), {"jwks_uri": "", "jwks": "{}"}
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_sp_oidc_metadata(self):
        ...
        SPAttribute.objects.create(
            attribute=self.attr_eppn, sp=self.user_sp, reason="User identification", oidc_userinfo=True
        )
